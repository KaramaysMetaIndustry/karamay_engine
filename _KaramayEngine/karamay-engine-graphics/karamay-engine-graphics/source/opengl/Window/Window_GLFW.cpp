...
void Window_GLFW::LoadContext()
{
	...
	if (PrimaryMonitor) {
		...
		bFullScreen = true;
		Window = glfwCreateWindow(PrimaryVideoMode->width, PrimaryVideoMode->height, 
...
void Window_GLFW::Tick(float DeltaTime)
{
	glfwSwapBuffers(Window);
	glfwPollEvents();
}

std::pair<size_t, size_t> Window_GLFW::GetFramebufferSize()
{
	return std::pair<size_t, size_t>(FrameBufferWidth, FrameBufferHeight);
}

void Window_GLFW::SwitchWindowed(int UpperLeftX, int UpperLeftY, int Width, int Height)
{
	auto PrimaryVideoMode = PrimaryMonitorMode.second;
	if (bFullScreen && Window && PrimaryVideoMode) {
		
		WindowedWidth = Width;
		WindowedHeight = Height;
		glfwSetWindowMonitor(Window, NULL, UpperLeftX, UpperLeftY, WindowedWidth, WindowedHeight, PrimaryVideoMode->refreshRate);
		bFullScreen = false;
	}
}

void Window_GLFW::SwitchFullScreen()
{
	auto PrimaryMonitor = PrimaryMonitorMode.first;
	auto PrimaryVideoMode = PrimaryMonitorMode.second;
	if (!bFullScreen && Window && PrimaryMonitor && PrimaryVideoMode) {
		glfwSetWindowMonitor(Window, PrimaryMonitor, 0, 0, PrimaryVideoMode->width, PrimaryVideoMode->height, PrimaryVideoMode->refreshRate);
		bFullScreen = true;
	}
}
...
{
	if (glfwGetWindowAttrib(Window, GLFW_MAXIMIZED) == GLFW_FALSE) {
...
bool Window_GLFW::IsFullScreen()
{
	return false;
}

std::pair<size_t, size_t> Window_GLFW::GetPrimaryMonitorPhysicalSize()
{
	int width, height;
...
void Window_GLFW::SetCallback()
{
	glfwSetWindowCloseCallback(Window, window_close_callback); // before window close
	glfwSetWindowSizeCallback(Window, window_size_callback); // when window size change
	glfwSetFramebufferSizeCallback(Window, framebuffer_size_callback); // when frame buffer change
	glfwSetWindowContentScaleCallback(Window, window_content_scale_callback); // when content scale change
	glfwSetWindowPosCallback(Window, window_pos_callback); // when window position change

	glfwSetKeyCallback(Window, key_callback); // when key input
	glfwSetCharCallback(Window, character_callback); // when char input
	glfwSetCursorPosCallback(Window, cursor_position_callback); // when mouse cursor position change
	glfwSetMouseButtonCallback(Window, mouse_button_callback); // when mouse button push
	glfwSetScrollCallback(Window, scroll_callback); // when mouse scroll roll
}

void Window_GLFW::SetInputMode()
{
	...
	glfwSetInputMode(Window, GLFW_LOCK_KEY_MODS, GLFW_CURSOR_DISABLED); //
