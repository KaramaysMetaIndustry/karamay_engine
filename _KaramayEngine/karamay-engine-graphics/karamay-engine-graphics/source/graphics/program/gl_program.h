...
public:
	...
	// set texture
	void set_texture_1d(std::uint32_t unit, const std::string& name, std::shared_ptr<gl_texture_1d> texture_1d) 
	{
		if (texture_1d) {
			glActiveTexture(GL_TEXTURE0 + unit);
			glBindTexture(GL_TEXTURE_1D, texture_1d->get_handle());
			set_uniform_1ui(name, unit);
		}
	}
	void set_texture_1d_array(std::uint32_t unit, const std::string& name, std::shared_ptr<gl_texture_1d_array> texture_1d_array) 
	{
		if (texture_1d_array) {
			glActiveTexture(GL_TEXTURE0 + unit);
	...
	void set_texture_2d(std::uint32_t unit, const std::string& name, std::shared_ptr<gl_texture_2d> texture_2d)
	{
		if (texture_2d) {
			glActiveTexture(GL_TEXTURE0 + unit);
	...
	void set_texture_2d_array(std::uint32_t unit, const std::string& name, std::shared_ptr<gl_texture_2d_array> texture_2d_array)
	{
		if (texture_2d_array) {
			glActiveTexture(GL_TEXTURE0 + unit);
	...
	void set_texture_3d(std::uint32_t unit, const std::string& name, std::shared_ptr<gl_texture_3d> texture_3d)
	{
		if (texture_3d) {
			glActiveTexture(GL_TEXTURE0 + unit);
	...
	void set_texture_cube(std::uint32_t unit, const std::string& name, std::shared_ptr<gl_texture_cube> texture_cube)
	{
		if (texture_cube) {
			glActiveTexture(GL_TEXTURE0 + unit);
	...
	void set_texture_cube_array(std::uint32_t unit, const std::string& name, std::shared_ptr<gl_texture_cube_array> texture_cube_array)
	{
	...
	void set_texture_buffer(std::uint32_t unit, const std::string& name, std::shared_ptr<gl_texture_buffer> texture_buffer)
	{
		if (texture_buffer) {
			glActiveTexture(GL_TEXTURE0 + unit);
			glBindTexture(GL_TEXTURE_BUFFER, texture_buffer->get_handle());
			set_uniform_1ui(name.c_str(), unit);
		}
	}

	// set uniform vec and matrix
	void set_uniform_1f(const std::string& name, glm::vec1 value)
	{
	...
	void set_uniform_3f(const std::string& name, glm::vec3 value)
	{
	...
	void set_uniform_1f(const std::string& name, std::float_t value) {}
	void set_uniform_2f(const std::string& name, std::float_t value0, std::float_t value1) {}
	void set_uniform_3f(const std::string& name, std::float_t value0, std::float_t value1, std::float_t value2) {}
	void set_uniform_4f(const std::string& name, std::float_t value0, std::float_t value1, std::float_t value2, std::float_t value3) {}

	void set_uniform_1d(const std::string& name, glm::dvec1 value)
	{
		glUniform1dv(glGetAttribLocation(_handle, name.c_str()), 1, glm::value_ptr(value));
	}
	void set_uniform_2d(const std::string& name, glm::dvec2 value)
	{
		glUniform2dv(glGetAttribLocation(_handle, name.c_str()), 2, glm::value_ptr(value));
	}
	void set_uniform_3d(const std::string& name, glm::dvec3 value)
	{
		glUniform3dv(glGetAttribLocation(_handle, name.c_str()), 3, glm::value_ptr(value));
	}
	void set_uniform_4d(const std::string& name, glm::dvec4 value)
	{
		glUniform4dv(glGetAttribLocation(_handle, name.c_str()), 4, glm::value_ptr(value));
	}

	void set_uniform_1i(const std::string& name, glm::ivec1 value)
	{
		glUniform1iv(glGetAttribLocation(_handle, name.c_str()), 1, glm::value_ptr(value));
	}
	void set_uniform_2i(const std::string& name, glm::ivec2 value)
	{
		glUniform2iv(glGetAttribLocation(_handle, name.c_str()), 2, glm::value_ptr(value));
	}
	void set_uniform_3i(const std::string& name, glm::ivec3 value)
	{
		glUniform3iv(glGetAttribLocation(_handle, name.c_str()), 3, glm::value_ptr(value));
	}
	void set_uniform_4i(const std::string& name, glm::ivec4 value)
	{
	...
	void set_uniform_2i(const std::string& name, std::int32_t value0, std::int32_t value1) {}
	...
	// set uniform buffer
	void set_uniform_buffer(std::uint32_t binding, const std::string& block_name, std::shared_ptr<gl_buffer> buffer)
	{
		if (buffer)
		{
	...
	void set_shader_storage_buffer(std::uint32_t binding, const std::string& block_name,  std::shared_ptr<gl_buffer> buffer)
	{
		if (buffer)
		{
