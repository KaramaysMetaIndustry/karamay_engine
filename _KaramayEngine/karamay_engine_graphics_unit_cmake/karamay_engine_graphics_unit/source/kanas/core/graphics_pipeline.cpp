...
bool kanas::core::graphics_pipeline::alllocate(const graphics_pipelineStateinitializer& Ininitializer, std::shared_ptr<pipeline_layout> InLayout, std::shared_ptr<pipeline_cache> InCache)
{
    ...
    PipelineCreateInfo.layout = InLayout->get_handle();
    PipelineCreateInfo.renderPass = Ininitializer.RenderPass->get_handle();
    PipelineCreateInfo.subpass = Ininitializer.SubpassIndex;
    PipelineCreateInfo.basePipelineHandle = VK_NULLhandle;
    PipelineCreateInfo.basePipelineIndex = -1;

    const VkResult GraphicsPipelineCreationResult = vkCreateGraphicsPipelines(get_device().get_handle(), InCache->get_handle(), 1, &PipelineCreateInfo, nullptr, &handle);
