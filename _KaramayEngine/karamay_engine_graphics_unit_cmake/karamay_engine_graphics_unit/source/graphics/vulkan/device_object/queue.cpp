...
bool queue::bind_sparse(const std::vector<VkBindSparseInfo>& bind_spare_infos, fence* f) noexcept
{
	vkQueueBindSparse(_handle, bind_spare_infos.size(), bind_spare_infos.data(), f->handle());
	return true;
}

bool queue::bind_sparse(const VkBindSparseInfo& bind_sparse_info, fence* f) noexcept
{
	vkQueueBindSparse(_handle, 1, &bind_sparse_info, f->handle());
	return true;
}

bool queue::present(const VkPresentInfoKHR& info) noexcept
{
