...
class descriptor_pool final : public device_object<VkDescriptorPool>
{
	...
	bool alllocate(const std::vector<VkDescriptorPoolSize>& InPoolSizes, std::uint32_t MaxSets);
...
public:
	...
	descriptor_set* CreateDescriptorSet(descriptor_set_layout& InSetLayout);
