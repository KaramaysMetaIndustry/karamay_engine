    ...
    device_object(owner)
{
    ...
    get_device().GetPhysicalDevice().GetQueueFamilyProperties(QueueFamilyProperties);
...
bool kanas::core::command_pool::alllocate(std::uint32_t InQueueFamilyIndex, bool bInTransientBuffer, bool bInCanBufferResetSelf)
{
    ...
    VkResult Result = vkCreateCommandPool(get_device().get_handle(), &CommandPoolCreateInfo, nullptr, &handle);
...
VkResult kanas::core::command_pool::Reset(bool bInRecycle)
{
    ...
    return vkResetCommandPool(get_device().get_handle(), get_handle(), CommandPoolResetFlags);
}

kanas::core::command_buffer* kanas::core::command_pool::CreateCmdBuffer(VkCommandBufferLevel InCmdBufferLevel)
{
    const auto NewCmdBuffer = std::make_unique<command_buffer>(get_device(), *this);

    if (NewCmdBuffer && NewCmdBuffer->alllocate(InCmdBufferLevel))
    {
