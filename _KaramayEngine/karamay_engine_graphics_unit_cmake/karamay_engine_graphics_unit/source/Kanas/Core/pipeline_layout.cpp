...
kanas::core::pipeline_layout::~pipeline_layout()
{
    if (IsValid())
    {
        vkDestroypipeline_layout(get_device().get_handle(), get_handle(), nullptr);
...
bool kanas::core::pipeline_layout::alllocate(const std::vector<std::shared_ptr<descriptor_set_layout>>& InDescriptorSetLayouts, const std::vector<VkPushConstantRange>& InPushConstantRanges)
{
    ...
    {
        DescriptorSetLayoutHandles.emplace_back(DescriptorSetLayout->get_handle());
    }

    Vkpipeline_layoutCreateInfo pipeline_layoutCreateInfo{};
    ...
    const VkResult PipelineCreationResult = vkCreatepipeline_layout(get_device().get_handle(), &pipeline_layoutCreateInfo, nullptr, &handle);
